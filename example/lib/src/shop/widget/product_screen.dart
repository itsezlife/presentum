...
class ProductScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const notFoundScreen = NotFoundScreen(message: 'Product is not found');
    ...
    return Scaffold(
      floatingActionButton: FavoriteButton.pulsating(productId: productId),
      body: SafeArea(
        child: CustomScrollView(
          slivers: <Widget>[
            ...
            RecommendationOutlet(
              ...
              subtitle: 'Similar products and recommendations',
              productId: productId,
            ),
...
class _ProductStarsState extends State<_ProductStars>
  ...
  void _rebuildIcons() {
    ...
    for (var r = 1; r < 11; r += 2) {
      if (rating10 > r) {
        _icons.add(
          const Icon(Icons.star, color: Colors.deepOrange, size: iconsSize),
        );
      } else if (rating10 == r) {
        _icons.add(
          const Icon(Icons.star_half, color: Colors.orange, size: iconsSize),
        );
      } else {
        _icons.add(
          const Icon(
...
class _ProductPhotosListView extends StatefulWidget {
  const _ProductPhotosListView({required this.product});
    ...
    return Center(
      child: SizedBox(
        ...
        child: Row(
          ...
          mainAxisAlignment: MainAxisAlignment.start,
          children: <Widget>[
            Text('$title: '.toUpperCase(), style: textTheme.labelMedium),
            const Expanded(
              child: CustomPaint(painter: _ProductPropertyDotsPainter()),
            ),
