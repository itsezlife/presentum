...
class CatalogBreadcrumbs extends StatefulWidget {
  ...
  factory CatalogBreadcrumbs.category({
    ...
    bool rebuilds = false,
    Key? key,
  }) => CatalogBreadcrumbs._(categoryId: id, rebuilds: rebuilds, key: key);
  ...
  factory CatalogBreadcrumbs.product({
    ...
    bool rebuilds = false,
    Key? key,
  }) => CatalogBreadcrumbs._(productId: id, rebuilds: rebuilds, key: key);
...
class _CatalogBreadcrumbsState extends State<CatalogBreadcrumbs> {
  ...
  List<(Widget, VoidCallback?)> _buildActions(
    ...
    void goToCatalog() {
      _router.setState(
        (state) => state
          ..arguments[const HomeAppTab().identifier] = HomeTabsEnum.catalog.name
    ...
    String getProductTitleById(Object? id) =>
        ShopScope.getProductById(context, switch (id) {
  ...
  Widget buildAction(Widget title, [VoidCallback? callback]) => Flexible(
    child: ConstrainedBox(
      ...
      child: TextButton(onPressed: callback, child: title),
    ),
  );
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  const chevron = SizedBox.square(
                    ...
                    child: Icon(Icons.chevron_right, size: chevronWidth),
                  );
                  final maxActionsWidth =
                      _actions.length * maxWidth +
  ...
  List<PopupMenuEntry<VoidCallback>> _builder() {
    ...
    return <PopupMenuEntry<VoidCallback>>[
      for (final action in widget.actions)
        PopupMenuItem<VoidCallback>(value: action.$2, child: action.$1),
    ];
  }
}
