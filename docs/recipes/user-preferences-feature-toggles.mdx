...
## 6) Guard (crucial): candidates → slots + enforce catalog + user prefs

This is the missing piece you called out.

- `setCandidatesWithDiff(...)` only updates **candidates**
- a guard must decide what becomes **active** and what goes to the **queue**
...
final class FeatureSchedulingGuard
  ...
  PresentumState<FeatureItem, AppSurface, AppVariant> call(
    ...
    for (final entry in bySurface.entries) {
      ...
      // - Settings: stable alphabetical order
      // - Other surfaces: priority order
      if (surface == AppSurface.settingsToggles) {
        items.sort((a, b) {
          final at = a.payload.metadata['title'] as String? ?? '';
          final bt = b.payload.metadata['title'] as String? ?? '';
